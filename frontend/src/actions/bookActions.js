...
import {
  ...
  BOOK_TOP_SUCCESS,
  BOOK_TOP_FAIL,
  BOOK_CATEGORY_REQUEST,
  BOOK_CATEGORY_SUCCESS,
  BOOK_CATEGORY_FAIL,
...
export const listCategoryBooks = (id, pageNumber = '') => async (dispatch) => {
  try {
    dispatch({ type: BOOK_CATEGORY_REQUEST })

    const { data } = await axios.get(
      `/api/books/category/${id}?pageNumber=${pageNumber}`
    )

    dispatch({
      type: BOOK_CATEGORY_SUCCESS,
      payload: data,
    })
  } catch (error) {
    dispatch({
      type: BOOK_CATEGORY_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
